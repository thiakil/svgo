...
function filter_this_item(item, plugins, filter_item_callback){
    ...
    async.eachSeries(plugins, function(plugin, next_plugin){
        if (plugin.active && plugin.fn(item, plugin.params) === false) {
            process.nextTick(next_plugin, plugin);//this should halt the run, use this plugin's object as err (though we can use whatever here)
        } else {
            process.nextTick(next_plugin);
        }
    }, function(err){
        if (err === undefined || err === null){
            process.nextTick(filter_item_callback, true);
        } else {
            process.nextTick(filter_item_callback, false);
        }
    })
}
        ...
        if (reverse) {
            ...
            filter_this_item(filter_item, plugins, function(filt_res){
                if (!filt_res) {
                    ...
                    return process.nextTick(filter_callback, filt_res);
                }
                if (filter_item.content && filter_item.elem != 'foreignObject'){
                    perItem(filter_item, plugins, reverse, function(){
                        process.nextTick(filter_callback, filt_res);
                    });
                } else {
                    process.nextTick(filter_callback, filt_res);
                }
