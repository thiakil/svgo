 ...
 * @return {String}
 */
JS2SVG.prototype.createDoctype = function(doctype_item) {

    var doctype_out = this.config.doctypeStart;

    if (doctype_item.content) {
        /*if (doctype_item.doctype.indexOf('[') > -1){
            var doctype_split = doctype_item.doctype.split(']');
            //doctype_split[doctype_split.length-2] += "\n";
            doctype_item.content.forEach((function(entity_item){
                doctype_split[doctype_split.length-2] += this.config.indent + entity_item.entity.raw + '\n';
            }).bind(this));
            //doctype_split[doctype_split.length-2] += "]";
            doctype_out += doctype_split.join(']');
        } else {*/
            doctype_out += doctype_item.doctype;
            doctype_out += ' [\n';
            doctype_item.content.forEach((function(doctype_item){
                if (doctype_item.sgml){
                    doctype_out += this.config.indent + '<!'+doctype_item.sgml+'>' + '\n';
                } else if (doctype_item.entity){
                    doctype_out += this.config.indent + '<!ENTITY '+doctype_item.entity.name+' "'+doctype_item.entity.value+'">' + '\n';
                    this.config.customEntities[doctype_item.entity.name] = doctype_item.entity.value;
                }
            }).bind(this));
            doctype_out += ']';
        //}
    ...
    doctype_out += this.config.doctypeEnd;
...
JS2SVG.prototype.createAttrs = function(elem) {
    ...
    elem.eachAttr(function(attr) {
        ...
        var attrVal = String(attr.value).replace(this.config.regValEntities, this.config.encodeEntity);

        for (var entity in this.config.customEntities){
            while (attrVal.indexOf(this.config.customEntities[entity]) > -1){
                attrVal = attrVal.replace(this.config.customEntities[entity], '&'+entity+';');
            }
        }
...
JS2SVG.prototype.createText = function(text) {
    ...
    for (var entity in this.config.customEntities){
        while (text.indexOf(this.config.customEntities[entity]) > -1){
            text = text.replace(this.config.customEntities[entity], '&'+entity+';');
        }
    }
