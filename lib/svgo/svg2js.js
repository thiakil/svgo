...
module.exports = function(data, callback) {
    ...
    sax.ondoctype = function(doctype) {
        ...
        //Illustrator files use entities for their namespaces, we need to add them to sax.ENTITIES for it to parse correctly and remove them with the EditorNS plugin
        ...
        while (ent = entity_internal.exec(doctype)){
            ...
            (docNode.content = docNode.content || []).push(new JSAPI(ent[0], docNode));
        }

        //remove the parsed entities from the doctype string. TODO: output the properly/conditionally
        docNode.doctype = docNode.doctype.replace(entity_internal, '').replace(/(\r?\n\s*\r?\n)*/gm, '').replace('[]', '');
    ...
    sax.onend = function() {
        ...
        else callback({ error: this.error });
        this.error = null;
    };
